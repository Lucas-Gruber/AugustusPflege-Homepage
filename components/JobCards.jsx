...
const JobCards = ({data}) => {
  return (
    <>
      {data.map((data) => (<JobsCard key={data.id} title={data.titel} aufgaben={data.aufgaben} beschreibung={data.beschreibung} />))}
    </>
  );
}
    ...
    <div className="relative w-full h-full p-5 bg-white rounded-md shadow-lg">
      ...
      <div className="my-5">
        {props.aufgaben.map((data) => (
          <div key={data.Aufgabe} className="flex items-center w-full pb-4 space-x-3 cursor-pointer dark:border-gray-700">
            <div>
              <ListArrowIcon  />
            </div>
      ...
      <div className="absolute left-0 right-0 bottom-5">
        <div className="m-auto">
          <Modal buttontext="Weitere Details" modalcontent={props.beschreibung} modalclose="Schließen" />
        </div>
      </div>
    </div>
  )
